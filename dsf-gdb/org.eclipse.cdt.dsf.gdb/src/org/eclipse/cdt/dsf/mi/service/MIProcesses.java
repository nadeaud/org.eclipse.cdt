...
//		unregister();
   ...
   public IThreadDMContext createThreadContext(IProcessDMContext processDmc, String threadId) {
	...
	@Override
	public void getHSAWorkGroups(IDMContext dmc, String axis, IDMContext hsaParent, DataRequestMonitor<IDMContext[]> rm) {
	...
	@Override
	public void setProcessSelection(ICommandControlDMContext ctx, RequestMonitor rm, String[] selections, IDMContext[] thCtx) {
		rm.setStatus(new Status(IStatus.ERROR, GdbPlugin.PLUGIN_ID, INVALID_HANDLE, "Unsupported Method", null)); //$NON-NLS-1$        	        			
		rm.done();	
	}
}
